...
- 🧠 **Пул моделей** (шестерёнка в шапке) — отметь чекбоксом, какие модели
  показывать кнопками быстрого переключения на главной; клик по кнопке сразу
  меняет модель дефолтной сессии. Новая модель в пуле помечена бейджем
  «новая» первые 14 дней.
- ⏱ **Кольцо-таймер до сброса окна** — на карточке каждого аккаунта, рядом с
  почтой: цвет кольца сам показывает срочность (зелёный → жёлтый → красный по
  мере приближения сброса 5-часового окна).
- ⌨️ **Кнопки `/compact` и новая сессия** — прямо на карточке активного
  аккаунта, без переключения в терминал: сжать контекст или начать `/clear`
  одним кликом (та же инъекция в `screen`-сессию, что и у кнопок переключения
  модели).
...
Что означает `429`/`400`/`403` на карточке аккаунта, когда каждый из них
реально возникает и что с ним делать — [ERRORS.md](ERRORS.md).
...
- **v1.10.2** — таймер «до PRO→FREE» мог остаться на старой дате после
  продления: якорь писался только при полностью чистом ответе кнопки
  «✅ Я продлил». Если в момент нажатия Anthropic ещё отдавал Free, или план
  уже был Pro, но usage-эндпоинт отвечал ошибкой (частый 429 сразу после
  оплаты), — дата не записывалась, а фоновый опрос позже присылал «снова PRO —
  вернул в ротацию», но якорь не трогал → карточка висела «⏳ сегодня».
  Теперь: кнопка пишет якорь при любом плане Pro/Max (ошибка usage не мешает),
  а фоновый опрос при переходе Free→Pro сам ставит якорь — на момент нажатия
  кнопки (если она нажималась в последние 6 ч) или на момент обнаружения.
  Заодно закрыт «холодный старт»: после первого естественного Free→Pro
  таймер появляется и без кнопки.
...
- **v1.10.0** — режим «оптимизация»: крайний-случай фолбэк (когда ни один
  ...
  проверялась только загрузка 5-часовой сессии — аккаунт с низким session%,
  но 100% недельного лимита, всё равно выглядел "лучшим" кандидатом,
  балансер переключался на него и застревал там навсегда (выбраться уже
  было некуда). Теперь недельный потолок (`weekly_cap`) проверяется и в этой
  ветке тоже, как и во всех остальных.
- **v1.9.9** — live-зеркало консоли: подчёркивание (`text-decoration:underline`)
  больше не рендерится вообще. На свежей (без `--continue`) сессии claude
  встречался терминальный баг, из-за которого SGR-underline "залипал" почти
  на всём экране (десятки % ячеек, включая статус-бар) — визуально выглядело
  как полосы под каждой строкой. Так как 100%-покрытие экрана подчёркиванием
  никогда не несёт полезного сигнала в этом зеркале, атрибут просто не
  рисуется — независимо от причины на стороне терминала.
- **v1.9.8** — live-зеркало консоли: геометрия окна (число строк/колонок)
  теперь измеряется честно через `ioctl(TIOCGWINSZ)` прямо на pty-устройстве
  screen-окна, а не "плотной коробкой" вокруг текста hardcopy (та плавала
  вместе с контентом). Замер идёт полностью в обход command-канала screen —
  никаких побочных эффектов на живой экран сессии при любой частоте опроса.
- **v1.9.7** — порог `weekly_cap` (недельный лимит, при котором аккаунт
  считается исчерпанным для режима оптимизации — и форсит уход с активного,
  и отсекает кандидата на переключение) поднят с 95% до 99%. На 95% ещё
  оставался заметный неиспользованный запас — аккаунт с почти пустой
  сессией, но неделей 95-98%, раньше вообще не мог стать целью переключения
  (кроме самого крайнего тира без учёта недели), хотя мог полезно поработать
  ещё немного. Чисто числовой порог, логика фильтра не менялась.
- **v1.9.6** — ссылка перелогина («🔑 Войти заново») теперь автоматически
  копируется в буфер обмена в момент открытия новой вкладки — не нужно
  вручную тянуться правой кнопкой мыши по ссылке, если вкладка открылась
  не в том профиле браузера или её заблокировал попап-блокер, ссылку
  можно сразу вставить куда нужно. Подсказка в диалоге ввода кода теперь
  об этом явно говорит.
- **v1.9.5** — таймер до PRO→FREE переехал в компактный чип «⏳ N дн» прямо
  в шапке карточки, сразу после тега плана, вместо отдельной строки под
  барами 5ч/недели — меньше вертикального места, красный акцент при
  оставшихся < 3 дней (title показывает точную дату). Чисто визуальный
  рефакторинг существующей v1.9.4-фичи, логика/якорь не менялись.
- **v1.9.4** — таймер до ожидаемого конца подписки на карточке аккаунта.
  Кнопка «✅ Я продлил» теперь, помимо мгновенной перепроверки плана, ещё и
  запоминает момент подтверждения (`state.json["renewal"][acc]`) — от него
  дальше считается «якорь минус 30 мин буфера + календарный месяц»
  (`dateutil.relativedelta`, не фиксированные 30 дней, чтобы дата не
  «уезжала» на длинных месяцах), и на карточке появляется строка «до
  ожидаемого PRO→FREE: ~N дн H ч (ДД.ММ)». Существующий мгновенный
  Telegram-алерт при реальном обвале Pro→Free никуда не делся — таймер
  просто ориентир заранее, а не замена уведомления. Заодно закрыт
  соседний баг того же хендлера: `/api/recheck` решал успех только по
  `plan`, игнорируя `error` — план и usage у Anthropic независимые
  эндпоинты, и план мог уже вернуться на Pro, пока usage ещё 429-ит;
  кнопка радостно писала «вернул в ротацию», хотя авто-переключение всё
  равно хард-скипает любой аккаунт с непустым `error`. Теперь при
  `plan ok + error` — отдельное честное сообщение без ложного «успеха».
  Новая зависимость: `python3-dateutil` (добавлена в `install.sh`).
  **Важно про холодный старт:** якорь пишется только внутри `/api/recheck`,
  а кнопка «✅ Я продлил» показывается только когда план уже упал в `free`.
  На свежей установке, пока аккаунт ни разу не падал в Free, строки таймера
  не будет — она появится только после первого естественного цикла
  Pro→Free, когда вы оплатите продление и нажмёте «Я продлил». До этого
  момента единственная страховка — мгновенный Telegram-алерт о реальном
  обвале Pro→Free (см. выше), он работает независимо от таймера.
- **v1.9.3** — фикс «застревания» будильника паузы: при подъёме сессии
  (`pause_ctl.py wake`) текст сообщения и Enter теперь шлются в screen
  ДВУМЯ раздельными вызовами `stuff` с короткой паузой между ними, а не
  одной командой `текст + "\r"`. Длинная строка, залитая одним `stuff`
  разом, могла осесть в поле ввода TUI непроглоченной — терминал принимал
  символы, но хвостовой `\r` не срабатывал как отправка (похоже на
  bracketed-paste: пачка байт, прилетевшая одним махом, трактуется как
  вставка текста, а не «напечатали и нажали Enter»). Короткие команды это
  не задевало — задевало только длинные пробуждающие сообщения.
- **v1.9.2** — ошибки на карточке аккаунта теперь переводятся в понятный
  русский текст вместо сырого `HTTP Error 400: Bad Request`: протухший токен
  («нажми «Войти заново»»), лимит частоты запросов (429), блокировка WAF
  (403), временная недоступность Anthropic (5xx), обрыв сети/таймаут — каждый
  случай с коротким советом, что делать. Разбор кода ошибки происходит один
  раз в момент перехвата исключения (у HTTP-ответа тело читается только
  один раз), неизвестные коды не гадаются, а показываются как есть с
  сохранённым числовым кодом. Подробный разбор причин по-прежнему в
  [ERRORS.md](ERRORS.md) — карточка теперь просто даёт короткую версию сразу.
- **v1.9.1** — два фикса фонового опроса: (1) `poll_loop()` теперь сам
  уважает `backoff_until` после пойманного 429 — раньше он звал
  `collect(force=True)` безусловно на каждом тике, а `force=True` обходит
  и 30-секундный дедуп, и бэкофф, так что фон продолжал стучаться в
  Anthropic и мог словить 429 заново ещё до истечения бэкоффа (регресс
  относительно фикса v1.8.0, который закрыл только не-force путь от
  панели); (2) если авто-переключаться совсем некуда (у всех аккаунтов
  сессия выше порога) — теперь приходит одно уведомление в Telegram на
  эпизод (не чаще раза в 2 часа) со статусом каждого аккаунта и временем
  ближайшего сброса, вместо полной тишины.
- **v1.9.0** — модалка моделей (⚙) стала одним списком: известные модели и
  найденные кандидаты (после «🔄 Проверить новые модели») показываются
  вместе, каждая строка — один чекбокс, который применяется сразу (известная
  модель включается/выключается, кандидат добавляется и включается одним
  кликом, отдельная кнопка «Добавить» больше не нужна). Список сортируется по
  семейству модели (Fable/Opus/Sonnet/…), Haiku — всегда последним как самая
  лёгкая линейка, внутри семейства новые версии — сверху.
- **v1.8.0** — после пойманного 429 от Anthropic любой не-force запрос
  (обычный опрос панели) блокируется на удвоенный `poll_sec`, а не только
  внутренний таймер фонового опроса. Раньше 30-секундный дедуп в `collect()`
  защищал лишь от одновременных вызовов: если панель опрашивала `/api/limits`
  реже 30с, но чаще, чем успевал притормозить фоновый опрос, она тут же
  повторяла запрос, который только что словил 429.
- **v1.7.0** — на карточке аккаунта: кольцо-таймер до сброса 5-часового окна
  (цвет по срочности — зелёный/жёлтый/красный) и, для активного аккаунта, две
  кнопки прямо в карточке — сжать контекст (`/compact`) и новая сессия
  (`/clear`), без похода в терминал. Плюс плашка «АКТИВНЫЙ» переехала в
  плавающий бейдж поверх верхней границы карточки (вместо инлайн-тега рядом с
  почтой) — так активный аккаунт виден с одного взгляда на список.
- **v1.6.0** — неактивные аккаунты не опрашиваются на каждом тике: если
  у аккаунта уже есть свежий кэш (не старше `inactive_poll_sec`, по
  умолчанию 300с) — реальный запрос к Anthropic пропускается, данные
  берутся из кэша с пометкой возраста (`stale_ts`). Активный аккаунт
  всегда опрашивается заново каждый тик — реакция на форс-свитч не
  замедляется. Актуально при небольшом `poll_sec` (быстрая реакция) —
  раньше это удваивало нагрузку на ВСЕ аккаунты разом и ловило
  `429 Too Many Requests` на неактивных. Кнопка «Я продлил»
  (`/api/recheck`) кэш обходит явно — там нужна гарантированная свежесть.
- **v1.5.0** — пул моделей с чекбоксами: шестерёнка в шапке открывает модалку
  со списком всех известных моделей, галочка вкл/выкл каждую из строки
  быстрых кнопок на главной — применяется сразу, без «Сохранить». Клик по
  кнопке модели переключает дефолт сессии (`POST /api/model`, эндпоинт был и
  раньше, но панель никак его не вызывала — переключить модель можно было
  только вручную командой `/model` в самой сессии). Новая модель в
  `MODELS` помечается бейджем «новая» на 14 дней (`model_added_ts` в
  `config.json`).
